    ...
    return (
      <>
        ...
        {topics.topics.map((topic, i) => (
          <li key={`topic-${i}`}>
            <p>
              <strong className="badge badge-primary">{topic.name}</strong> <strong>{topic.count || 1}</strong> possible references to this topic
              {topic.url && (
                <>
        ...
        {topics.keywords.map((keyword, i) => (
          <li key={`keyword-${i}`}>
            <p>
              <strong className="badge badge-primary">{keyword.name}</strong> <strong>{keyword.count || 1}</strong> mentions of this keyword
              {keyword.url && (
                <>
